
//# publish
module 0xCAFE::AdditionModule {
    public fun add_and_return_sum(a: u8, b: u8): u8 {
        let sum = a + b;
        sum
    }

    public fun lambda_add_and_multiply(a: u8, b: u8): (u8, u8) {
        let lambda: |u8, u8| (u8, u8) has copy+drop = |x: u8, y: u8| {
            let added = x + y;
            let multiplied = x * y;
            (added, multiplied)
        };
        lambda(a, b)
    }
}


//# publish
module 0xCAFE::NestedInlineCaller {
    use 0xCAFE::AdditionModule;

    public inline fun inline_addition(a: u8, b: u8): u8 {
        AdditionModule::add_and_return_sum(a, b)
    }

    public fun call_nested_inline(a: u8, b: u8, c: u8): u8 {
        let first_sum = inline_addition(a, b);
        let second_sum = AdditionModule::add_and_return_sum(first_sum, c);
        second_sum
    }
}


//# run 0xCAFE::AdditionModule::add_and_return_sum --args 10u8 20u8


//# run 0xCAFE::AdditionModule::lambda_add_and_multiply --args 3u8 5u8


//# run 0xCAFE::NestedInlineCaller::call_nested_inline --args 7u8 8u8 9u8


// Featurres:
// e0d671ef5e56e3c17cc028f58558e133: Test that the Move function correctly computes the addition of two u8 values before returning a specific value.
// 02a0d162ad59e2910b98dd07a8d9cb41: Write functions containing lambda (anonymous function) expressions.
// 9f38a2013886067cef890651dcf5f9f0: Test that calling an inline function from one module within another module correctly performs the nested function calls and returns the expected result.
