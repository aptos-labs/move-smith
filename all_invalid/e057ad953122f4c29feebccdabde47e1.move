...
module 0xCAFE::LambdaAndParsingTests {
    ...
    public fun parse_move_source(source: vector<u8>): (vector<vector<u8>>, vector<vector<u8>>) {
        ...
        while (i < len) {
            // Find next newline
            let j: u64 = i;
            // Loop to find newline
            while (j < len) {
                // index the source at position j
                let c = *vector::index(&source, j);
                if (c == 0x0A) {
                    break;
                }
                j = j + 1;
            }
            // Extract line from i to j
            let line = vector::slice(&source, i, j);
            // Check for documentation comment '//'
            if (vector::length(&line) >= 2 && 
                *vector::index(&line, 0) == 0x2F && // '/'
                *vector::index(&line, 1) == 0x2F) // '/'
            {
                vector::push_back(&mut docs, line);
            } else {
                vector::push_back(&mut lines, line);
            }
            // move i to j + 1 to skip newline
            i = j + 1;
        }
        (lines, docs)
    }
