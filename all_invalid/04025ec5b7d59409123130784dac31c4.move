...
module 0xCAFE::TestModule {
    ...
    fun inner_loop(s: &mut State) {
        ...
        let i = 0u64; // Variables need to be mutable to reassign
        while (i < 3) {
            ...
            while (j < 2) {
                j = j + 1;
            }; // Semicolon to end the while statement
            // Increment outer 'i'
            i = i + 1;
        }; // Semicolon to end the while statement
    ...
    public fun run_loops(s: &mut State): () {
        ...
        let outer_var_table = s.outer_var + s.counter; // Removed 'as' to fix syntax
        for i in 0..2 {
            // Shadowed variable inside for
            let outer_var = outer_var_table + i;
    ...
    struct EventCollector has store {
