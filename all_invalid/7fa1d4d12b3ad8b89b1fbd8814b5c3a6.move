...
module 0xCAFE::AdvancedFeaturesTest {
    ...
    public fun variable_shadowing_test() {
        ...
        let counter = 0u8;
        loop {
            let counter_inner = counter; // create a new inner variable with different name to test shadowing
            if (counter_inner >= 3) {
                break;
            };
            let counter = counter_inner + 1; // shadow the outer `counter`
            // loop variable shadowing - ensuring no side effects
        };
...
//# run 0xCAFE::AdvancedFeaturesTest::apply_closure --args true --args 7u8


//# run 0xCAFE::AdvancedFeaturesTest::apply_closure --args false --args 7u8
