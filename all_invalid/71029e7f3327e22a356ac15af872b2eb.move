...
module 0xABCD::FeatureTest {
    ...
    // Function to test local variable initialization inside and outside loop
    ...
    public fun handle_special_types(): (unit, unresolved_error) {
        ...
        let res = ((), /* simulate error, but just use unresolved error placeholder */);
        result = res;
        result
    }

    // Helper to simulate an unresolved error (mock)
    struct unresolved_error has copy, drop {}
