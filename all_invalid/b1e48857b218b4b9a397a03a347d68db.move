...
module 0xCAFE::GenericAccessControl {
    ...
    /// Getter function type alias for reading
    public type ReadFn<R> = &Container<R, ResourceW> * ( &signer ) -> u64;

    /// Setter function type alias for writing
    public type WriteFn<W> = &mut Container<ResourceR, W> * ( &signer, u64 ) -> ();

    /// A public function to read value from the Container using a function value as permissioned read.
    public fun read_with_access_control<R: store>(s: &signer, container_ref: &Container<R, ResourceW>, read_fn: ReadFn<R>): u64 {
        read_fn(container_ref, s)
    }

    /// A public function to write value to the Container using a function value as permissioned write.
    public fun write_with_access_control<W: store>(s: &signer, container_mut_ref: &mut Container<ResourceR, W>, new_val: u64, write_fn: WriteFn<W>) {
