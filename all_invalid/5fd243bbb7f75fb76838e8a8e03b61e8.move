...
module 0xBADD::InteractionTest {
    ...
    // Move internal functions outside of a function for proper declaration
    // Since Move does not support nested functions, we declare internal functions at module level
    fun hidden_func(): u64 {
        42
    }
    ...
    public fun script_entry() {
        ...
        // After inner block
        // Verify outer_var remains unchanged (no runtime check, just for syntax)
        outer_var
    }
    ...
    public fun assign_and_modify() {
        ...
        let i = 0;
        while (i < 3) {
            let i = i + 1; // shadowing i, new variable
            counter = counter + i;
            i = i + 1; // update i for next iteration
        };
