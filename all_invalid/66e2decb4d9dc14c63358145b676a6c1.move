...
module 0xCAFE::OperatorPrecedence {
    ...
    spec module {
        ...
        axiom immutable_ref:
            (forall<&u64>((r: &u64) => *r >= 0));

        axiom mutable_ref_modification:
            (forall<&mut u64>((r: &mut u64) => { *r = *r + 1; true }));

        axiom nested_containers:
            (forall<Container<&u64>>((c: Container<&u64>) => (*c.val != 0)));
    }
}
