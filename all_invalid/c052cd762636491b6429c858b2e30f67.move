...
module 0xCAFE::LambdaVector {
    ...
    /// A struct that stores vector of function pointers taking u64 and returning u64
    /// NOTE: Move does not support storing lambdas or function pointer types in vectors.
    /// So we must replace this with an approach using an enum approach or manual dispatch.
    /// But since example tests storing functions, we will simulate with an enum of variants representing the functions.
