...
module 0xCAFE::HexLiteralSpecTest {
    ...
    invariant {
        // The error was with invariant [T: copy] which is not supported.
        // Here replaced with a generic-style invariant patterns without type param.
        true
    }

    // invariant update cannot have generic params like this; removing
    // Instead leave empty spec update as syntax check
    spec update_invariant() {
        // dummy spec update, no op
    }

    // Axiom with generic type T and unsigned literals (similarly removed generic param)
    axiom {
        true
    }
