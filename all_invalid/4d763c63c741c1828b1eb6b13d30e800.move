...
// # run
script {
    // In Move, there is no direct std library function `move_std::language_storage::module_id`
    // Instead, we can just demonstrate calling the version function.
    // Alternatively, the script can just call the version function directly.
