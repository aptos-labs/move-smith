...
module 0xCAFE::AdvancedFeatureTest {
    ...
    // Move does not have 'internal' visibility. To restrict access within the module,
    // simply declare the function as 'private' (default) or 'public' for external access.
    // Here, we want internal only, so make it private (no specifier).
    fun get_value_from_inner(s: &Container): u64 {
    ...
    pure fun spec_has_value(v: u64): bool {
        v > 0
    }
    ...
    public fun shadow_variable_demo(initial: u64): u64 {
        ...
        let x = 0u64; // 'x' needs mut for reassignment
        while (x < 3) {
