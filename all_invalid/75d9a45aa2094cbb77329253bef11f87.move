...
module 0xCAFE::InteractionTests {
    ...
    public fun test_read_via_reference(signer_addr: address) {
        ...
        let val = read_value_ref(&borrow_global<PrivateStruct>(signer_addr).value);
...
//# run 0xCAFE::InteractionTests::run_all_tests
...
// 7d44e1d1ff8ebaad5e10f4b26a25e664: Use the 'Test' attribute to mark functions as test cases, which can be removed in production builds if testing functions are not to be kept.
