...
module 0xCAFE::AbilitiesInParams {
    ...
    // Original error: local `x` of type `KeyType` does not have drop, so consume it by unpacking
    ...
    // Original error: local `x` of type `StoreType` does not have drop, so consume it by unpacking
