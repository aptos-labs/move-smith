...
module 0xCAFE::TestLambda {
    ...
    // This tests Move lambda support: For Aptos Move, anonymous functions use the 'move' keyword

    public fun lambda_runner() {
        // Aptos Move anonymous function syntax uses 'move'; non-standard fun() syntax is invalid
        let add = move |x: u64, y: u64| { x + y };
        let result = add(10, 20);

        let neg = move |x: u8| { 0 - x }; // test unary minus (allowed for unsigned; checks calculation)

        // Compose lambdas
        let composed = move |a: u64, b: u64| {
            let sum = add(a, b);
            let doubled = move |z: u64| { z * 2 };
            doubled(sum)
        };
