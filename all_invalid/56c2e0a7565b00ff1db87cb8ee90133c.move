...
module 0xCAFE::TestControlFlowAndPatternMatching {
    use std::vector; // This alias is unused, but as per warning, it can be left or removed if desired.

    // Define the enum E used in matching functions
    // Move the enum inside this module for scope correctness
    public enum E {
        V1,
        V2(x: u8, y: u8),
        V3 { a: bool },
