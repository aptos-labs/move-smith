...
module 0xBADD::TestModule {
    ...
    public fun handle_status<E: copy>(status: Status<E>): u64 acquires {
        match (status) {
            Status::Active => 1,
            Status::Inactive => 2,
            Status::Pending => 3,
            Status::Custom(e) => {
                ...
                // For testing, extract inner value if Status is Custom with nested container
                match (e) {
                    _ => 42,
                }
            }
    ...
    public fun trigger_diag_error() acquires {
        let _err_code = ERROR_CODE_DIAG;
        // Simulate an error scenario: assign invalid value intentionally
        // Since Move does not support runtime errors, simulate by abort
        abort(ERROR_CODE_DIAG as u64);
    }
