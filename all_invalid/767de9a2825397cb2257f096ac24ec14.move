...
module 0xCAFE::OperatorPrecedence {
    ...
    public fun runner(s: signer) {
        ...
        let (a, b, flag, counter) = read_fields(&s);
        mutate_fields(&s);
...
// fc490319c2e43b6d55d271181b4a55ec: Test that functions can correctly read global resource fields with various read and write permissions, and validate that permission annotations (reads and writes) are enforced appropriately within the Move language.
