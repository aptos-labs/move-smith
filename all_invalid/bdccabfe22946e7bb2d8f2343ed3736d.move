...
module 0xCAFE::UniqueModulesCollection {
    ...
    public fun add_module(c: &mut Collection, name: vector<u8>, version: u8) acquires Collection {
        ...
        let module_info = ModuleInfo { name: name.clone(), version };
...
module 0xCAFE::FakeParserParser {
    ...
    public fun parse_source(source: &vector<u8>): bool {
        ...
        while (pos < source_len) {
            let slice = vector::sub_range(source, pos, pos + vector::length(invalid_keyword));
...
script {
    ...
    spec script {
        invariant [ "x always less than 100" ] (x: u64) { x < 100 }
    }

    fun main() {
        debug::print(b"Running script with attribute, constant, function, spec");
...
// 4cae262dcda7dc51d5ee2b7734878a5e: Provide diagnostics and error reporting for invalid Move source code during parsing.
