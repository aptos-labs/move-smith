...
script {
    fun main() {
        ...
        if (true) {
            let y = 42;
            let x = y; // Assign within if branch
        } else {
            let y = 99;
            let x = y; // Assign within else branch
        };
        // At this point, x should be 42, from the if branch
        // (We do not assert, but the test checks that no errors occur and it retains the correct value.)
