...
module 0xDEAD::ControlFlowTest {
    ...
    public fun test_nested_if_continue(flag: bool): bool {
        let result = false; // result must be mutable to be assigned
        let i = 0; // i must be mutable to update
        while (i < 5) {
    ...
    public fun validate_address_assignment(addr_str: vector<u8>): bool {
        let count_eqs = 0; // count must be mutable
        let len = vector::length(&addr_str);
        let idx: u64 = 0; // idx mutable
        while (idx < len) {
    ...
    public fun test_nested_blocks() {
        let x: u64 = 10;
        let y: u64; // y must be mutable
        let outer_x = x; // outer_x mutable to modify

        {
            // Inner block with mutable reference
            let inner_ref: &mut u64 = &mut outer_x;
