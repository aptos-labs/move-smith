...
module 0xCAFE::NestedMatch {
    ...
    public fun runner(): u64 {
        // outer 0:
        let s = 0u64
            + (nested_match_handler(0, true) as u64)
            + (nested_match_handler(0, false) as u64)
            + (nested_match_handler(4, true) as u64) // outer 1 with inner (0,y)
            + (nested_match_handler(4, false) as u64)
            + (nested_match_handler(1, true) as u64) // outer 1 with inner (1,y)
            + (nested_match_handler(1, false) as u64)
            + (nested_match_handler(2, true) as u64) // outer 2 case
            + (nested_match_handler(2, false) as u64);
        s
    }
}
...
script {
    ...
    fun main() {
        ...
        aptos_debug::print(&vector::empty<u8>()); // no-op print just for coverage
        let gs_res = 0xCAFE::GenericStructTest::runner();
        let cv_res = 0xCAFE::ConstVecTest::runner();

        // Print results mod 256 as u8 bytes
        aptos_debug::print(&vector::singleton((nm_res as u8)));
        aptos_debug::print(&vector::singleton(gs_res));
        aptos_debug::print(&vector::singleton(cv_res));
    }
}