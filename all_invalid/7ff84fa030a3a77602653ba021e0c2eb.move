...
module 0xCAFE::FeatureTestModule {
    ...
    public fun complex_integer_operations(): u64 {
        let a: u8 = 0x1F;          // 31
        ...
        // Cast b_div to u8 (since 857 > u8 max), truncate to u8 (which is 1 byte)
        let b_div_u8 = b_div as u8; // 857 mod 256 = 857 - 3*256=857-768=89
        let result1 = a_shifted & b_div_u8; // (124 & 89) = 124 & 89

        // Nested operations with multiple types
        let c_and_u64 = c_and as u64; // cast to u64
        let nested_result = c_and_u64 | d_shifted;

        nested_result
    }

    // Test control flow with return inside nested loops and conditionals
    public fun control_flow_test(): u8 {
        let x: u8 = 0;
