...
module 0xCAFE::AdvancedFeaturesTest {
    ...
    public fun add_generic<T: copy + drop + std::add::Add<Output = T>>(a: T, b: T): T {
        a + b
    }

    // Wrapper function that passes a function as argument and invokes it
    public fun use_function<F: copy + drop + (fn(T, T): T), T: copy + drop + std::add::Add<Output = T>>(f: F, a: T, b: T): T {
        f(a, b)
    }
