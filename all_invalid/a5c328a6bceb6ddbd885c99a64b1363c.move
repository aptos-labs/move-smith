...
module 0xCAFE::GenericStructEnum {
    ...
    public fun runner() {
        ...
        // Replace `match` with `switch` for variant matching
        switch me {
            MyEnum::VariantOne(_) => {},
        ...
        // Use conditional guards with if statements since 'switch' doesn't support guards
        let me2 = MyEnum::VariantOne(42u64);
        if (matches_variant_one_gt_40(&me2)) {
            // do nothing, just test guard
        } else if (matches_variant_one(&me2)) {
            // do nothing, just test guard
        } else {
            // fallback
        }
    ...
    // Helper function to check if me2 matches MyEnum::VariantOne(v) with v > 40u64
    public fun matches_variant_one_gt_40(me: &MyEnum<u64>): bool {
        switch *me {
            MyEnum::VariantOne(v) if v > 40u64 => true,
            _ => false,
        }
    }

    // Helper function to check if me2 matches MyEnum::VariantOne(_)
    public fun matches_variant_one(me: &MyEnum<u64>): bool {
        switch *me {
            MyEnum::VariantOne(_) => true,
            _ => false,
        }
    }
}
