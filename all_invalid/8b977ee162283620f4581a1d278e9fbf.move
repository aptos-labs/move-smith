...
module 0xCAFE::ExpControl {
    ...
    public fun configure_experiments(exps: vector<u8>) {
    ...
    public fun runner() {
        ...
        configure_experiments(vector[]);
        configure_experiments(vector[b"feature_x=on"]);
    }
}
...
module 0xCAFE::TokenMatcher {
    ...
    public fun is_token_eq(tok: Token, target: Token): bool {
        match tok {
            Token::Identifier => match target {
                Token::Identifier => true,
                _ => false,
            },
            Token::Number => match target {
                Token::Number => true,
                _ => false,
            },
            Token::Symbol(c1) => match target {
