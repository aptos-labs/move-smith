module 0xDEAD::TestModule {
    ...
    public fun test_quantifiers(x: u8, b: bool): bool {
        ...
        // Note: In Move, 'if' expressions can be used directly in variable initialization.
    ...
    public fun test_copy_consumption_conditions(flag1: bool, flag2: bool): u64 {
        ...
        if (flag1) {
            // use num_copy
            let consumed_num = copy num_copy;
            // do something with consumed_num
            consumed_num
        } else if (flag2) {
            // clone or re-assign the copy
            let reused_num = copy num_copy; // copy semantics in Move allow this
            reused_num
        } else {
            // fallback, return 0
            0
    ...
    public fun test_return_followed_by_unary(): u8 {
        let x: u8 = 5;
        ...
        if (x > 0) {
            ...
            let neg_y: i8 = - (copy y as i8); // convert to signed to negate
            // return neg_y as u8, casting back
            neg_y as u8
        }
    }

    /// Additional test to verify the compiler handles `return` immediately followed by expressions with unary/reference operators.
    public fun test_return_unary_expression(): u16 {
        let a: u16 = 10;
        if (a > 5) {
