...
module 0xCAFE::AdvancedFeatureTest {
    ...
    fun internal_function_a() acquires Self {
        // Internal logic here
        42
    }

    // A private function, should not be accessible outside
    fun internal_private() acquires Self {
        99
    }

    // A public function to test access from scripts
    public fun call_internal_function() acquires Self {
        internal_function_a()
    }
    ...
    public fun variable_shadowing_and_reassignment(): u64 {
        ...
        if (true) {
            let x = 20u64;
            let y = x + 5;
            // After this block, x should be the outer x
...
// 402535e82fa622d23a841f68ab33cdef: Test that a value moved through multiple local variables retains its value after one variable is reassigned, ensuring proper local variable kill and liveness semantics.
