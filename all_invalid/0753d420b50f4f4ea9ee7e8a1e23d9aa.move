...
module 0xCAFE::TestStringAndLoop {
    ...
    public fun test_loop_assign_param(x: u8): u8 {
        ...
        while (i < 5u8) {
            val = val + i;
            i = i + 1;
            // val is updated every iteration to val+i
            val;
        };
        val
    }
    ...
    public fun runner_disallow() {
        let lambda: &fn(&fn(u8): u8): u8 = &fun (g: &fn(u8): u8): u8 {
            g(5u8) + 1u8
        };
...
//# run 0xCAFE::TestStringAndLoop::literal_length --args 0x48656c6c6f5c6e576f726c64  # "Hello\nWorld", length excluding '\n' escape counted as one char 
