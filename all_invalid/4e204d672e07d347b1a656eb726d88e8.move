...
// 3. Flushing writes with `0x1::storage::flush` for storage ordering.
...
module 0xCAFE::SpecAndInlineTest {
    ...
    public fun demo(addr: &signer) acquires Counter {
        ...
        let _nval = Self::compute_native(counter.val);

        // Increment again to check ordering after flush
        Self::fast_increment(a, 7);
        storage::flush();
    }
