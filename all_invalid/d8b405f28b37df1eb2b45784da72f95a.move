...
module 0xDEAF::AdvancedFeaturesTest {
    ...
    public fun variable_scope_test(): u64 {
        let outside = 10u64;
        let x = outside; // Shadow variable for inner loop, mutable needed for reassignment
        let y: u64 = 0;
        let i: u64; // Declare outside loop variable for reuse
        i = 0;
        while (i < 3) {
            let inside = i as u8; // Inner scope shadowing
            x = outside + i; // Remove 'as' casting error by using implicit conversion
            y = y + inside as u64; // cast inside value to u64
            i = i + 1;
        };
        // verify that outside remains unchanged
        assert!(outside == 10u64, 100);
        // verify the final values
        assert!(x == 12u64, 101); // when i=2, outside + 2 = 12
        assert!(y == 3u64, 102);
        y
    }
