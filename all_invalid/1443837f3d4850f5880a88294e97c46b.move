...
// 1. Lexical scoping/capturing/shadowing via inline functions.
// 2. Dotted expressions (struct field access).
// 3. Functions with the maximum possible number of parameters (65).
...
module 0xCAFE::ShadowAndFields {
    ...
    public fun foo(x: &mut u64, f: &mut (fun(&mut u64))) {
        f(x); // function modifies x (by reference)
        let _ = *x;
    }

    // Companion runner that tests shadowing/capturing.
    public fun runner() {
        // First: x not mutated in caller's scope
        let mut x = 1u64;
        Self::foo(&mut x, &mut Self::set_3);
        // x here should be 3 now, since reference is passed
        assert!(x == 3, 100);
        // Second: direct call again just to ensure reference mutation is visible
        let mut y = 9u64;
        Self::foo(&mut y, &mut Self::set_7);
        assert!(y == 7, 101);
    }
    public fun set_3(p: &mut u64) { *p = 3; }
    public fun set_7(p: &mut u64) { *p = 7; }
    ...
    // 3. Function with 65 parameters (u8s). No lambdas, just direct sum.
    public fun sum_65(
