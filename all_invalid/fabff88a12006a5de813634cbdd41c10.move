...
module 0xCAFE::LambdaShadowTest {
    ...
    // The correct type syntax for function pointers is `&fun(T): R` without `&` in the param
...
module 0xCAFE::vector {
    ...
    public fun length<T>(_v: vector<T>): u64 {
        // _v is unused, but vector<T> doesn't have drop ability and cannot be dropped
        123
    }
