...
module 0xCAFE::InteractionTest {
    ...
    public fun script_entry_1(s: signer): u64 {
        ...
        let local_var: u64 = counter.value; // made mutable
        // Shadowed variable with same name inside loop
        let local_var_shadowed: u64; // declare mutable if needed; not used afterwards

        let i: u64 = 0; // made mutable for iteration
        while (i < 3) {
            // Assign to original variable
            local_var = local_var + i;
            // Shadowing local variable within loop
            let local_var_shadowed: u64 = local_var * 2; // shadowed variable, could be omitted if unused

            // Call internal function
            internal_increment(&mut *counter); // dereference borrow_mut pointer
            i = i + 1;
        };
    ...
    fun internal_subtract(counter: &mut Counter, delta: u64) {
        counter.value = counter.value.saturating_sub(delta); // fix: remove internal_subtract, not used in scripts
    }
