...
module 0xCAFE::SpecRewriteTest {
    ...
    // Inline accessible function (remove 'public' visibility, inline functions cannot be public)
    inline fun accessible_inline_function(x: u8, y: u8): (u8, u8) {
        // Returns tuple
        (x + 1, y + 1)
    }

    // Inline non-accessible function
    inline fun non_accessible_inline_function(x: u8): u8 {
        x * 2
    }
