...
module 0xCAFE::TestInteraction {
    ...
    // Public script function that tries to call internal function - should fail at verification if attempted outside
    public(script) fun call_internal() {
        internal_func();
...
module 0xCAFE::AccessRestriction {
    ...
    // Entry script to test calling internal functions (Should not compile if externally called)
    public(script) fun test_internal_access() {
        internal_only();
    }
}
