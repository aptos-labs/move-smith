...
module 0xCAFE::MathOps {
    ...
    public fun make_closure(x: u8): |u8| u8 has copy+drop {
        ...
        let closure: |u8| u8 has copy+drop = move (y: u8) {
            x + y
        };
        closure
    }
...
module 0xCAFE::ClosureTests {
    ...
    public fun closure_with_capture(x: u8): u8 {
        let c: |u8| u8 has copy+drop = move (y: u8) {
            x + y
        };
        c(10u8)
    }
...
// bf76c5a9eb009eaea679df9a799c295d: Allow the attachment of the original function's specification block to the generated spec function during inlining, enabling richer specs when inlining.
