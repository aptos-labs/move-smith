...
module 0xCAFE::AssignSpecTest {
    spec module {
        // Module spec variable (not const) to allow assignment in spec
        global spec(VAR_SPEC_CONST: u64);

        spec fun assign_module_const() {
            // We simulate assignment in spec by assigning to the spec variable
            VAR_SPEC_CONST = 42;
        }
    }
...
//# run
script {
    const SCRIPTCONSTVALIDNAME: u8 = 7;
    const SCRIPTCONSTUNDERSCORE: u64 = 123;
    //const 3INVALID: u8 = 9; // invalid name, commented out to compile
    //const invalid-with-dash: u8 = 5; // invalid name, commented out

    fun main() {
        let x = SCRIPTCONSTVALIDNAME;
        let y = SCRIPTCONSTUNDERSCORE;
