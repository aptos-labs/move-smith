...
module 0xCAFE::FriendModulesB {
    ...
    public fun increment_data(s: signer, initial_val: u64): u64 {
    ...
    public fun pattern_loop_example(): u64 {
        let mut sum = 0u64;
        let arr = vector[1u64, 2, 3, 4, 5];
        let len = vector::length(&arr);
        let mut i = 0u64;
        while (i < len) {
            let val = *vector::borrow(&arr, i);
            if (val % 2 == 0) {
                sum = sum + val;
            } else {
                sum = sum + 1;
...
// Featurres:
// a47b11ae668a4ba281b444bb785c12f2: Define modules with associated friends to specify access control and collaboration.
