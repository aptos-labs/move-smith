...
module 0xBADD::ShadowTest {
    ...
    public fun loop_and_shadow() {
        let x = 0u64;
        // To shadow, we create a new variable with the same name
        let x = x; // Inner x shadows outer x
        let y = 10u64;
        let y = y; // Shadow y

        let i = 0u64; // Need mut to modify in loop
        while (i < y) {
            // Shadow inner x in loop block
            let x = x + i; // Shadow outer x
            i = i + 1;
        };
        // After loop, outer x remains unchanged
        debug::print(&b"Outer x after loop:\n"[..]);
        debug::print_u64(x); // Should be 0

        // Verify inner x shadowing does not affect outer x
