//# publish
address 0xCAFE {
    //#[pragma(author = b"AptosTest", version = 1u8, tested = true)] // pragma attribute is currently not recognized by Move compiler; commenting out.
    module TestPragmaAndGas {
        ...
        public fun runner_loop_forever(account: &signer): () {
            // Call the internal function to cause infinite loop
            loop_forever();
        }

        // A function to test unit return
        public fun test_unit_return(account: &signer): () {
            let d = Dummy {};
            do_nothing();
            takes_dummy(d);
        }
