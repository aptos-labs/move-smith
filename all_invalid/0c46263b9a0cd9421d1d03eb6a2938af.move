...
module 0xCAFE::ContinueAndQuantifier {
    ...
    fun example_quantifier<T>() {
        let x: T; 
        // The variable `x` is declared with a generic type parameter T.
        // We won't initialize or use x here, since this is a compile-time test of quantifier syntax.
    }

    // Test 3: Define and use script in Move module
    public(script) fun run_script() {
        let x: u8 = 42;
        let y = 43u8;
        let z = x + y;
        let _ = z;
...
//# run 0xCAFE::ContinueAndQuantifier::example_quantifier --args 0x1
