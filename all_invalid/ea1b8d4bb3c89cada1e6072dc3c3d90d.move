...
module 0xCAFE::TestSuite {
    ...
    public fun script_test_variable_scope_and_loops(s: &signer) {
        ...
        while (outer_var < 3) {
            ...
            let inner_var = outer_var;

            // Reassign inner_var inside inner loop
            while (inner_var < outer_var + 2) {
                let inner_var = inner_var + 1;

                // Shadowing inner_var with same name, ensure scope isolation
                let inner_var = inner_var + 1;

                // After inner loop, outer_var should be unchanged
                // and inner_var should be incremented multiple times
            }

            // Check that outer_var incremented correctly
