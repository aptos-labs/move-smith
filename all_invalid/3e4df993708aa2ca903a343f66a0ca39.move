...
module 0xCAFE::DeepNestedLambda {
    ...
    public fun map_bytes(v: vector<u8>, f: &fun(u8): u8): vector<u8> {
    ...
    /// A function that returns a function that returns a function that increments by amount
    /// Demonstrates deep nested returned functions (curried)
    public fun make_adder(amount: u8): fun(u8): fun(u8): u8 {
        // first function returns a function that adds amount
        fun first(x: u8): fun(u8): u8 {
            fun second(y: u8): u8 {
                (x + y + amount)
            };
            second
