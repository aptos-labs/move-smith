...
module 0xCAFE::ModuleCycle1 {
    // Remove friend references to modules not yet published to avoid unbound module errors.
...
script {
    ...
    fun main() {
        ...
        let outer_flag = true;
        let outer_count = 0u64;
        let inner_count = 0u64;

        // Remove illegal redeclarations with mut keyword
        // Variables in Move are mutable by default (no mut needed)
        let outer_flag = outer_flag;
        let outer_count = outer_count;
        let inner_count = inner_count;
