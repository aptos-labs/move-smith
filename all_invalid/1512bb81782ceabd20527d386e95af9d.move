...
module 0xCAFE::IntegrationTest {
    ...
    public fun test_variable_scope_and_shadowing(s: address) {
        ...
        let shadow_var = 0;
        let (mut _i) = (0u64,); // emulate mutable variable outside
        while (_i < 3) {
        ...
        // Shadowed variable outside loop should not be accessible, but we simulate access
        // (In Move, shadowing restricts scope, so no access; here just for illustration)
        // No direct access to shadow_var outside loop as per spec, so just avoid usage
