...
module 0xBADD::TestBindingAndGroups {
    ...
    public fun group_under_address_block() {
        // Use the address block without the 'address' keyword
        // in Move, the correct syntax is 'address <addr> { ... }'
        // So remove 'address' keyword and just use the syntax directly
        0xCAFE {
            let signer_ref = signer::borrow_global<signer>(@0xCAFE);
            let obj = TestStruct {id: 999, data: vector::empty<u8>()};
            move_to<TestStruct>(&signer_ref, obj);
        }
        0xBADD {
            let signer_ref = signer::borrow_global<signer>(@0xBADD);
            let obj2 = TestStruct {id: 888, data: vector::empty<u8>()};
            move_to<TestStruct>(&signer_ref, obj2);
        }
    }
