...
module 0xCAFE::TestModule {
    ...
    public fun script_variable_scope(s: signer, init_value: u64) {
        ...
        while (outer_var < 5) {
            let shadow_outer_var = outer_var * 2; // shadow outer_var inside loop
            inner_shadowed_var = shadow_outer_var + 10;
            // Update outer_var for next iteration
            outer_var = outer_var + 1;
        }

        // outer_var here should be unchanged (shadowed locally within loop)
    ...
    public fun script_variable_assignment(s: signer) {
        ...
        // after loop, count should be 0 + (5 + 10 + 15) = 30
        ()
    }
