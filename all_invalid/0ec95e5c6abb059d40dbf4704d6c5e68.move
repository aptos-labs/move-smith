...
script {
    fun test_nested_if_continue() {
        ...
        loop {
            ...
            // increment counter to eventually reach break condition
            counter = counter + 1;
        };
...
module 0xCAFE::ControlFlowTest {
    ...
    public fun test_lambda_capture() {
        ...
        // Test that only persistent functions or public functions are captured
        // Private/internal functions cannot be captured; in Move, lambdas can't capture non-public functions directly.
        // So this test assumes the lambda does not capture private/internal functions.
    }
...
//# run 0xCAFE::ControlFlowTest::get_u8_from_enum --args E::V2(5, 10)
    ...
    // Note: To run with args, need to specify in CLI as: --args "E::V2(5,10)"
