...
module 0xDEAD::TestInteractions {
    ...
    public fun run_tests() {
        ...
        let c = 0u64;
        let c_mut = &mut c; // mutable reference to c
        while (a < 5) {
            // Shadowing variable 'a'
            let a = a + 1;
            // Assign to mutable c via reference
            *c_mut = a + b;
            // Call internal function (should not be callable from outside, but valid here)
            let _ = internal_helper(); // internal function call
            // 'a' shadowed; original 'a' remains unchanged
        };
        // Variables outside loop remain unchanged
        let _ = a;
        let _ = *c_mut;
        ...
        if (b > 8) {
            abort(7); // This abort terminates execution
        } else {
            // unreachable
            let _ = 0;
        };
