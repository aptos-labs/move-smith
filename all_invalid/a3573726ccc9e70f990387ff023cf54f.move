...
module 0xCAFE::InvariantTest {
    ...
    // Define an invariant that enforces counter is always non-negative (u64, so always true, but constructed to enforce invariant syntax)
    invariant [0xCAFE::InvariantTest::Counter] {
        // The invariant ensures that `count` is always >= 0 (which is always true for u64, just for test)
