...
module 0xCAFE::TupleAndStructTest {
    ...
    // Function returning a tuple with values copied from input argument fields.
...
module 0xCAFE::ReferenceTest {
    ...
    // Function that accepts a struct with mutable references and modifies them
    // Since tuple types like (&mut u8, &mut u16) are not supported as local variables or parameters,
    // we'll create a struct to hold the mutable references instead.
    struct MutRefs has store {
        x_ref: &mut u8,
        y_ref: &mut u16,
    }

    public fun update_tuple_of_mut_refs(t: &mut MutRefs, new_x: u8, new_y: u16) {
        *(t.x_ref) = new_x;
        *(t.y_ref) = new_y;
    }
}
