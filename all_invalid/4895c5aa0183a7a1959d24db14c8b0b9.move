...
module 0xCAFE::TestModule {
    ...
    public fun validate_binary_header(binary_data: vector<u8>): bool {
        ...
        let header: vector<u8> = vector[0x0A, 0x0B, 0x0C, 0x0D];
        // Ensure binary_data is at least 4 bytes
        if (vector::length(&binary_data) < 4) {
            return false;
        };
        let prefix = vector::slice(&binary_data, 0, 4);
        vector::equals(&prefix, &header)
    }
