...
module 0xCAFE::AbilitiesInParams {
    ...
    // Unpack consumes x since it doesn't have drop.
    ...
    public fun run_drop() {
        ...
        // d has been consumed by accept_drop_param, so to call accept_drop_plain we must use d again
        // but d cannot be reused. Instead, create a fresh value for second call:
        let d2 = DropType {value: 10};
        let y = accept_drop_plain(d2);
        let _ = x + y;
    }
}
...
module 0xCAFE::OptionalAccess {
    ...
    // Add has key to PrivateRes so it can be stored under an address and borrowed as global
    struct PrivateRes has store, key {
        val: u64
    }
    ...
    // Fixed: Add lifetime on signer param to relate return reference lifetime to input
    // Aptos Move enforces referencing rules that require return references to be related to params
    public fun borrow_private<'a>(s: & signer): & 'a PrivateRes {
        borrow_global<PrivateRes>(signer::address_of(s))
    }

    // Public function to borrow PublicRes reference
    public fun borrow_public<'a>(s: & signer): & 'a PublicRes {
