...
module 0xCAFE::TupleAndString {
    ...
    public fun trim_leading_spaces(s: &vector<u8>): Indices {
        ...
        let start = 0u64;
        // use while-like loop with mut variable
    ...
    public fun slice_trimmed_string(s: &vector<u8>, indices: Indices): vector<u8> {
        // vector::sub_vector is the correct function for slicing (start inclusive, end exclusive)
        vector::sub_vector(s, indices.start, indices.start + indices.length)
    }
    ...
    public fun runner() acquires Indices {
        // Use vector::from_utf8 or string::utf8 function to create vector<u8> from string literal
        let s = string::utf8(b"   Move Language");
