...
//# publish
module 0xCAFE::ComplexStructs<T: copy + drop> {  
    // A struct with attributes, abilities, type parameter and layout annotation.
    ...
    #[derive(copy, drop, store)]
    struct AnotherStruct has store, copy {
        inner: MyStruct<u8>,  // nested struct with concrete param
        flag: bool,
    }

    // Runner function to instantiate and use the structs.
    public fun run_structs() {
        let s: MyStruct<u8> = MyStruct { a: 1, b: 100, c: 255 };
...
script {
    ...
    fun main() {
        ...
        let s: ComplexStructs::MyStruct<u8> = ComplexStructs::MyStruct { a: 5, b: 15, c: 42 };
