...
module 0xCAFE::TestModule {
    ...
    public fun scope_test(flag: bool): (u64, u64) {
        let outer_var: u64; // should be mutable to build correctly
        let inner_var: u64; // declare inner_var outside loop, will shadow inside

        // Initialize outer_var
        outer_var = 10;

        // Declare inner_var outside; it will be shadowed inside loop
        // Shadow inner_var in inner scope
        while (flag) {
            ...
            // Break condition to avoid infinite loop if flag is true (for test)
            // Since no condition to break, forcibly break to prevent timeout
            break;
        };
        // After loop, inner_var is shadowed; use a new variable for outside
    ...
    public fun pure_add(x: u64, y: u64): u64 {
        ...
        assert!(x + y >= x, 1000);
        // Postcondition: result is sum (not enforced at runtime, but for testing)
        x + y
    }
    ...
    public fun attempt_invalid_access(s: signer): u64 {
        ...
        // This code is commented out because it would cause a compile error if uncommented.
        // 0xCAFE::SomePrivateModule::internal_function()
        // For the test, just return 0.
        0
    }
