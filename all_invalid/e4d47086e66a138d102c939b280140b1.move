...
module 0xCAFE::ImportAliasModule {
    use 0xCAFE::AbilityConstrainedModule as ACM;

    // Create a local alias for struct HasCopyOnly
    use ACM::HasCopyOnly as HCO;

    // Create another alias for the generic struct CopyHolder
    use ACM::CopyHolder as CHolder;
    ...
    public fun call_accept_copy() {
        let val = HCO { val: 123 };
        let _c = ACM::accept_copy<HCO>(val);
    }
}
...
module 0xCAFE::CombinedTest {
    use 0xCAFE::AbilityConstrainedModule as ACM;
    use 0xCAFE::LintAttributesModule as LAM;
    use 0xCAFE::ImportAliasModule as IAM;
    ...
    public fun combined_function<T: copy + drop>(val: T) {
        ...
        IAM::alias_generic_test();

        // Call function from lint attribute module that has mutation lint skipped
        let data = LAM::Data { counter: 0 };
        LAM::mutate_counter(&mut data);

        // Use generic function with ability constraint from AbilityConstrainedModule
        let _holder = ACM::accept_copy<T>(val);
    }

    public fun combined_runner() {
        let value = ACM::HasCopyOnly { val: 11 };
        combined_function<ACM::HasCopyOnly>(value);
    }
}
