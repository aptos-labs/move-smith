...
module 0xCAFE::FriendUser {
    ...
    public fun pattern_destructure_complex_tuple() {
        ...
        // In Move, nested tuple destructuring requires you to destructure step by step:
        let (x, y_and_z, bytes, flag) = t;
        let (y, z) = y_and_z;
    ...
    public fun pattern_destructure_enum() {
        ...
        // Since Move doesn't support direct enum destructuring in let binding,
        // simulate with match plus let inside the branches

        if (matches!(v2, SampleEnum::Variant2(_, _))) {
            let (a, b) = (10u8, true);
            let _ = a;
            let _ = b;
        };

        if (matches!(v3, SampleEnum::Variant3 { data: _ })) {
