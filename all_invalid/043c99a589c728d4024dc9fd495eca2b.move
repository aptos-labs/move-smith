...
module 0xCAFE::ListFormatter {
    ...
    // Format a vector<u8> as comma-separated decimal numbers as a string
    public fun format_u8_list(list: vector<u8>): vector<u8> {
        ...
        while (i < len) {
            ...
            // Convert u8 to decimal string bytes by repeated division
            let dec_str = u8_to_string(v);
            vector::append(&mut result, dec_str);
            if (i != len - 1) {
    ...
    // Format a vector<vector<u8>>, joining inner lists formatted as u8 lists by semicolons + spaces
    public fun format_nested_u8_list(nested: vector<vector<u8>>): vector<u8> {
        ...
        while (i < len) {
            ...
            let inner_fmt = format_u8_list(*inner);
            vector::append(&mut result, inner_fmt);
            if (i != len - 1) {
                vector::push_back(&mut result, b';' as u8);
                vector::push_back(&mut result, b' ' as u8);
            };
...
//# run 0xCAFE::ListFormatter::format_nested_u8_list --args vector<vector<u8>> [vector[u8][], vector[u8][3], vector[u8][255,0,10]]
...
// The above is a valid layering: DepModB depends on DepModA and ListFormatter, which are valid.

// The earlier CycleA and CycleB create a cyclic use dependency for testing error detection.


// Featurres:
// 89e8cf8e461333926733be700897af1a: Format lists of values as comma-separated strings
...
// 31dc48d79b0be2a227aebc7cbf22a62f: Use unpacked structs by pattern matching with the 'Unpack' variant.
