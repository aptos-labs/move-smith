...
module 0xDEAD::InteractionTestModule {
    ...
    internal fun internal_increment_state(state_ref: &mut State) {
        state_ref.counter = state_ref.counter + 1;
    }

    // Internal function that should not be callable externally
    internal fun internal_set_flag(state_ref: &mut State, value: bool) {
...
module 0xC0FF::VariableShadowing {
    ...
    public fun assign_var_in_loop() {
        ...
        let temp_var: u64 = 0;
        for (i in 0..5) {
            let temp_var: u64 = i * 2; // shadowed within loop
            temp_var = temp_var + 1; // modify inner temp_var
