...
module 0xDEAD::TestModule {
    ...
    public fun variable_scope_test(initial_x: u8): u8 {
        ...
        // Note: Move does not support shadowing with 'let inner_x' inside the loop directly,
        // so we must reassign 'inner_x' each iteration after declaring it outside.
        // To simulate shadowing, we declare inside the loop.
        while (inner_x < 5) {
    ...
    public fun test_inline_fn(): u8 {
        let x = 5u8;
        // Move the closure to correct syntax
        let lambda = |a: u8| -> u8 {
            a + 10
        };
