...
module 0xCAFE::DependencyGraph {
    ...
    // Check if node present by name (returns index or u64::MAX)
    public fun find_node(addr: &signer, name: &vector<u8>): Option<u64> {
        let graph = borrow_global<Graph>(signer::address_of(addr));
        let len = vector::length(&graph.nodes);
        let i = 0u64;
        while (i < len) {
            let node_ref = &vector::borrow(&graph.nodes, i as u64);
...
// Featurres:
// ff1ff5914e804698506d874569466b6c: Add additional module nodes to the dependency graph, even if they have no dependencies.
