...
module 0xCAFE::ExperimentManager {
    ...
    public fun runner(): (bool, bool) {
        let mut experiments = vector::empty<vector<u8>>();
        let good = b"teleport";
        let bad = b"unknown";
        let ok_added = Self::try_register_experiment(&mut experiments, copy good);
        let bad_added = Self::try_register_experiment(&mut experiments, copy bad);
        (ok_added, bad_added)
    }
}
...
script {
    ...
    fun main() {
        ...
        let mut attempted: vector<vector<u8>> = vector::empty<vector<u8>>();
        let exp1 = b"fusion";
        let exp2 = b"blaster"; // Not in EXPERIMENTS

        let added1 = ExperimentManager::try_register_experiment(&mut attempted, copy exp1);
        let added2 = ExperimentManager::try_register_experiment(&mut attempted, copy exp2);

        // Update/modify a variable
        let total_attempted = vector::length(&attempted);
