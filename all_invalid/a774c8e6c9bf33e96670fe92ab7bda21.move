...
module 0xCAFE::SpecModule {
    ...
    spec module {
        // Declare a type parameter invariant: for all T, the value inside Container<T> must be non-zero for u8
        invariant forall<T> (c: Container<T>) {
            exists<u8>(c.value) && (c.value != 0)
        }
        // Actually Move spec does not allow axiom with conditionals like this.
        // But for testing, we write an axiom with assoc with type parameters
