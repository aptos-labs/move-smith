...
module 0xCAFE::AccessSpecifiers {
    ...
    friend struct FriendResource has key, store {
        value: u64,
    }
    ...
    friend entry fun create_friend_resource(account: signer, val: u64) {
    ...
    friend fun read_friend_resource(account: signer): u64 {
...
module 0xCAFE::LambdaPass {
    ...
    public entry fun nested_lambda(
        s: signer,
        f_outer: |(|u64|u64 has copy + drop, u64)|u64 has copy + drop,
        f_inner: |u64|u64 has copy + drop,
        input: u64
    ): u64 {
    ...
    public fun higher_order_example(): u64 {
        ...
        let outer_lambda: |(|u64|u64 has copy + drop, u64)|u64 has copy + drop = |f: |u64|u64 has copy + drop, y: u64| {
...
module 0xCAFE::AliasingTest {
    ...
    public entry fun call_lambda_functions(
        account: signer,
        lambda: |u64|u64 has copy + drop,
        nested_lambda_fn: |(|u64|u64 has copy + drop, u64)|u64 has copy + drop,
        input: u64
    ): u64 {
...
script {
    ...
    fun main(s: signer) {
        ...
        let outer_lambda: |(|u64|u64 has copy + drop, u64)|u64 has copy + drop = |f, y| { f(y) + 5 };
...
script {
    ...
    fun outer(f: |u64|u64 has copy + drop, x: u64): u64 {
        f(x) * 2
    }

    fun main(s: signer) {
        let lambda: |u64|u64 has copy + drop = simple;
        let nested_lambda: |(|u64|u64 has copy + drop, u64)|u64 has copy + drop = outer;
