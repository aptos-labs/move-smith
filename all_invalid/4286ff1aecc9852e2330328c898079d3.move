...
address 0xCAFE {
    module NumericSuffixes {
        ...
        public fun runner() {
            let _nums = Self::init_numbers();
            // Assigned to _nums to avoid unused variable warning
        }
    }
}
...
address 0xCAFE {
    module DerefAndField {
        ...
        // Removed borrow_s as returning reference to local not allowed
...
//# publish
address 0xCAFE {
    module GenericFieldRef {
        struct Wrapper<T> has copy, drop, store {
            val: T,
        }

        public fun new<T>(v: T): Wrapper<T> {
            Wrapper { val: v }
        }

        public fun get_ref<T>(wrapper: &Wrapper<T>): &T {
            &wrapper.val
        }

        public fun dup_and_check<T: copy + drop + store + eq>(wrapper: Wrapper<T>): bool {
