...
module 0xCAFE::AnnotationFormatterTest {
    ...
    struct FormatterRegistry has store, key {
        formatter: option::Option<fun(): ()>,
    }

    // Initialize the registry
    public fun initialize_registry() {
        let reg = FormatterRegistry { formatter: option::none() };
        move_to<FormatterRegistry>(&signer::address_of(&signer), reg);
    }
    ...
    public fun register_formatter() {
        ...
        let dummy_fun: fun(): () = dummy_formatter;
    ...
    // Use wildcard '*' in spec (simulate with a special value, e.g., 0xFF)
    public fun spec_with_wildcard() {
        // Suppose '*' is represented by u8::max_value()
        let wildcard: u8 = u8::max_value();
        let specific_value: u8 = 42;

        // Specification pattern with simulated wildcard
        if (specific_value == wildcard) {
            // This block is for the wildcard (won't run since 42 != 255)
        } else {
        ...
        let input_value: u8 = u8::max_value(); // 255
        if (input_value == wildcard) {
