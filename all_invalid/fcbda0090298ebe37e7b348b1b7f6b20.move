...
module 0xCAFE::QuantifierTest {
    ...
    public fun exists_val(v: &vector<Item>, x: u8): bool acquires Item {
    ...
    public fun forall_lt(v: &vector<Item>, limit: u8): bool acquires Item {
        let len = vector::length(v);
        let i = 0u64;
        let res = true;
        while (i < len) {
    ...
    public fun choose_val(v: &vector<Item>, x: u8): u64 acquires Item {
        let len = vector::length(v);
        let i = 0u64;
        while (i < len) {
            if (vector::borrow(v, i).val == x) {
    ...
    public fun test_quantifiers(n: u8, x: u8, limit: u8) {
        ...
        assert!(!bool::xor(x < n, exists_val(&v, x)), 1000);
...
// Featurres:
// c7d99c85e4a0160e247b886ab5351028: Specify a module's address in code with the correct address value.
