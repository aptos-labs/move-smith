...
module 0xBEEB::TestNestedFields {
    ...
    public fun create_outer() acquires OuterStruct {
...
module 0xCAFE::ControlFlowTest {
    ...
    public fun shadow_and_modify(s: &mut TestStruct): u64 {
        let count = s.count;
        {
            ...
            count = count_shadow; // modify outer variable within block
        }
...
// Attempt to access internal function from outside module (should fail if not allowed; this line simulates attempt)
// This line is illustrative; in practice, trying to access internal_secret from outside would be compile error
// Nonetheless, testing visibility restrictions conceptually
