...
module 0xDEAD::NestedControlFlow {
    ...
    public fun generate_diagnostics(outer_var: u64, inner_var: u64): vector<vector<u8>> {
        ...
        let msg1 = vector::append(b"Diagnostic: outer_var=", vector::empty<u8>()); // Changed to correct function
        vector::append(&mut msg1, u234_to_bytes(outer_var));
        vector::push_back(&mut messages, msg1);

        let msg2 = vector::append(b" inner_var=", vector::empty<u8>()); // Changed to correct function
    ...
    public fun variable_scope_test(flag: bool): u64 {
        ...
        while (i < 5) {
            ...
            let inside_var = i as u64; // Shadowed variable
            i = i + 1;
        }; // END of while
