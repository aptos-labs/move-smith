...
module 0xCAFE::SpecModule {
    ...
    spec module {
        // Correct axiom syntax: use `axiom` without colon and no trailing semicolon
...
script {
    ...
    fun main() {
        // Create a mutable data directly
        ...
        // Consume the `data` variable to avoid drop error by unpacking it:
        let RegularModule::Data { val: _ } = data;
    }
}
...
script {
    ...
    fun main() {
        ...
        let _x = NamedAddrModule::get_x(&holder);

        // Consume the `holder` variable to avoid drop error by unpacking it:
        let NamedAddrModule::Holder { x: _ } = holder;
    }
}
