...
module 0xCAFE::Queue {
    ...
    public fun enqueue<T: copy + drop>(q: &mut Queue<T>, item: T) {
        ...
        if (q.back == u64::MAX) {
            ...
            let i = q.front;
            // Copy elements from front to back into new_data starting at 0
            let j = 0;
            while (i + j < q.back) {
                vector::push_back(&mut new_data, *vector::borrow(&q.data, (i + j) as u64));
                j = j + 1;
            };
            q.data = new_data;
            q.front = 0;
    ...
    public fun runner_fifo(): vector<u8> {
        ...
        let v = vector::push_back(v, x0);
        let x1 = dequeue(&mut q);
        let v = vector::push_back(v, x1);
        let x2 = dequeue(&mut q);
        let v = vector::push_back(v, x2);
        v
    }
