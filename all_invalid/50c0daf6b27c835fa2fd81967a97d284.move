...
script {
    fun main() {
        ...
        // Create a nested block simulating different scopes
        {
            ...
            let i = 0u64;
            let i_mut = i; // To allow mutation
            while (i_mut < 3) {
                // Shadow variable 'i' inside the loop
                // Note: Move variables declared with 'let' are immutable by default
                // To simulate shadowing and mutation, declare a new variable
                let i = i_mut + 1; // Shadowed 'i' inside the loop
                // Assert inner shadowed 'i' has correct value
                assert!(i == (i),  "Inner shadowed i should be incremented");
                // Mutate 'i_mut' for next iteration
                i_mut = i;
            };
