...
//# publish
script {
    // Dereference reference to access value
    let dummy_ref = &0xCAFE::TestDereferenceAndExpressions::create_dummy();
    let value_ref = 0xCAFE::TestReferences::get_value_reference(dummy_ref);
    let dereferenced_value = *value_ref;

    // Use expression list and decimal numbers
    let result = 0xCAFE::TestExpressions::compute();
