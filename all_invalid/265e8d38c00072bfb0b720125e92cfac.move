...
module 0xCAFE::SpecWithTemplate {
    ...
    spec container_never_empty<T>(c: &Container<T>) {
...
module 0xCAFE::SpecWithTemplate2 {
    ...
    spec pair_ordered<T: copy + drop + store + key, U: copy + drop + store + key>(p: &Pair<T, U>, cmp: &fn(&T, &U): bool) {
...
module 0xCAFE::SpecLambdas {
    ...
    spec data_spec(d: &Data, threshold: u8) {
...
script {
    ...
    fun main() {
        // Use 0xCAFE as the signer (local test signer)

        let signer_ref = signer::address_to_signer(0xCAFE);

        SpecWithTemplate::runner(&signer_ref);
        SpecWithTemplate2::runner(&signer_ref);
        SpecLambdas::runner(&signer_ref);
    }
}