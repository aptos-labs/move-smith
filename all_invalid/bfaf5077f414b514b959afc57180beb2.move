...
module 0xCAFE::AdvancedFeaturesTest {
    ...
    // Function to test impure call in a spec annotation (conceptual)
    // Note: Move does not support calling impure functions in specs, so the test is conceptual
    public fun test_impure_in_spec(): bool {
        // This would be a specification assertion calling impure_func(), which is invalid
        // For the purpose of this code, just return true
        true
    }
    ...
    public fun complex_scenario(): u64 {
        ...
        let (a_value, b_value, data_vector) = get_nested_fields(
            OuterStruct {
