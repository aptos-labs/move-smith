...
module 0xCAFE::TypeHandlingTest {
    ...
    public fun test_vectors() {
        let vec_of_u8: vector<u8> = vector::singleton(1u8);
        let vec_of_bytes: vector<u8> = b"Hello\nWorld"; // Byte string literal

        // Vector literal with explicit type argument
        let vec_explicit: vector<u8> = vector::singleton(4u8); // or use vector::empty<u8>() and add elements
        // Note: To create a vector with multiple elements, use vector::into or push
    }
