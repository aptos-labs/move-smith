...
module 0xBEEFBEEFBEEFBEEFBEEFBEEFBEEFBEEFBEEFBEE {
    ...
    public fun access_and_assign() {
        ...
        // Note: Move does not support pattern matching like in some languages, so emulate by manual extraction
        let nested_vec = ns.nested_field;
        let (_first, _second) = match vector:: borrow(&nested_vec, 0) {
            key => (key, match vector:: borrow(&nested_vec, 1) {
                key => (key),
            }),
        };

        // Use the nested field in a computation
        let sum_keys = u64::from(*vector:: borrow(&ns.nested_field, 0)) + u64::from(*vector:: borrow(&ns.nested_field, 1));
...
module 0xBEEFBEEFBEEFBEEFBEEFBEEFBEEFBEEFBEEFBEE_TokenSyntacticErrors {
    ...
    public fun missing_semicolon() {
        ...
        // Note: in actual Move, syntax errors are caught at compile time, but for testing, we simulate expected failure
    }
