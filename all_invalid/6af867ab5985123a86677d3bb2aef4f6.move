...
module 0xCAFE::test_module {
    ...
    public fun test_reference_and_mut() {
        let x = 10;
        let ref_x = &mut x; // move x by reference mutably
    ...
    public fun test_order_of_evaluation() {
        ...
        // Correcting the 'if' expression with parentheses instead of braces for the condition
        let result = if (x + 1 > y) { 1 } else { 0 };
        assert(result == 0, 103);
        assert(x == 5, 104); // x is immutable here, so x does not change

        // Use &mut reference and block together
        let x_mut = x; // Need a mutable copy of x
        let ref_x = &mut x_mut;
        *ref_x = *ref_x + 2;
        assert(x_mut == 7, 105);
    }
