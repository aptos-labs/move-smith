...
module 0xCAFE::DiagnosticsAndArithmeticTest {
    ...
    public fun log_record(record: &Record) {
        ...
        let id_str = debug::formatted("{:#x}", &record.id);
        let name_str = debug::formatted("{}", &record.name);
        let flag_str = debug::formatted("{}", &record.flag);
        let data_str = debug::formatted("{}", &record.data);
        debug::print(&debug::formatted(
            "Record(id: {}, name: {}, flag: {}, data: {})",
    ...
    public fun syntax_error_test() {
        // Deliberate syntax errors - left as comments because Move won't compile with them
    ...
    public fun arithmetic_and_bitwise_tests() {
        ...
        // Log results
        debug::print(&debug::formatted("Shift left u8: {:#x}", &res_shift_left));
        debug::print(&debug::formatted("Division u64: {}", &res_div));
        debug::print(&debug::formatted("Modulo u128: {}", &res_mod));
        debug::print(&debug::formatted("Addition u64: {}", &res_add));
        debug::print(&debug::formatted("Subtraction u8: {}", &res_sub));
        debug::print(&debug::formatted("Cast u8 to u64: {}", &res_cast));
        debug::print(&debug::formatted("AND (0b1010 & 0b1100): {:#b}", &res_and));
        debug::print(&debug::formatted("OR (0b1010 | 0b0101): {:#b}", &res_or));
        debug::print(&debug::formatted("XOR (0b1111 ^ 0b0000): {:#b}", &res_xor));
    ...
    public fun combined_test() {
        ...
        debug::print(&debug::formatted("Shifted {:#b} to {:#b}", &val, &shifted));
    }
}
