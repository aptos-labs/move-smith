...
module 0xCAFE::EnumFieldAccessTest {
    ...
    public fun test_short_circuit_or(s: signer): bool {
        ...
        // Use `if` to enforce short-circuit manually, because Move does not have short circuit boolean operators
        if (lhs) {
            true
        } else {
            false_with_effect(s)
    ...
    public fun test_short_circuit_and(s: signer): bool {
        ...
        let lhs = false_with_effect(s); // emits event, result false
        // If lhs is false, no rhs evaluation
        if (!lhs) {
            false
        } else {
            true_with_effect(s)
