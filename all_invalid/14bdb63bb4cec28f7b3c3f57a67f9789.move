...
module 0xCAFE::MapEnumLabelTest {
    ...
    public fun labeled_blocks_test(): u64 {
        ...
        // insert some entries without labeled block because Move does not have labeled block statements, only labeled loops
        insert_or_update(&mut map, b"fileA", 101);
        insert_or_update(&mut map, b"fileB", 202);
        insert_or_update(&mut map, b"fileC", 303);

        // early exit with labeled loop (simulate labeled block using a loop that runs once)
        'insert_block: loop {
            ...
            break; // exit loop normally if not broken earlier
        };
        ...
        'outer: for (i in 0..vector::length(&map.keys)) {
            ...
            'inner: loop {
                ...
                break; // break inner loop to avoid infinite loop if no breaks triggered
            };
        };
