...
script {
    ...
    fun main() {
        ...
        TestModuleA::add_neighbor(&mut node1, addr_node2);
        TestModuleA::add_neighbor(&mut node2, @0xDEAD); // another fake neighbor

        let _count1 = TestModuleA::get_neighbor_count(&node1);
        let _count2 = TestModuleA::get_neighbor_count(&node2);

        // consume node1 and node2 by unpacking to avoid drop errors
        let TestModuleA::Node { value: _, neighbors: _ } = node1;
        let TestModuleA::Node { value: _, neighbors: _ } = node2;
