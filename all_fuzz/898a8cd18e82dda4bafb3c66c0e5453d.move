...
module 0xCAFE::FeatureFlags {
    ...
    public fun disable_feature(s: signer, feature_bit: u64) {
        ...
        let flags_ref = borrow_global_mut<Flags>(address);
        // Fixed operator: use ^ (bitwise xor) to flip the feature_bit bits to 0:
        // flags_ref.bits & !feature_bit is invalid because !feature_bit is not bool negation but bitwise negation isn't allowed as ! is boolean NOT.
        // In Move, bitwise negation is done by `!` operator but it requires std::! implemented. Actually, Move supports `!` on integers as bitwise negation.
        // The error suggests problem with `!feature_bit`. The proper syntax is to cast feature_bit to bitwise negation with `!`, but error happens.
        // The error suggests that `!` is being considered invalid. We can fix it by importing std::bitwise for `!` if needed or do: flags_ref.bits = flags_ref.bits & ((u64::MAX) ^ feature_bit);
        // We'll choose this for clarity.

        let all_ones = 0xffff_ffff_ffff_ffffu64;
        flags_ref.bits = flags_ref.bits & (all_ones ^ feature_bit);
    }
