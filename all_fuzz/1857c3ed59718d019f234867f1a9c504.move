...
module 0xCAFE::PropertySets {
    ...
    public fun runner() {
        ...
        let Car { running, .. } = person.car;
        let House { rooms, .. } = person.house;

        let _running_status = running;
        let _house_rooms = rooms;

        // Consume person by unpacking to avoid drop error (alternatively could also let Person { .. } = person;)
        let Person { name: _, age: _, car: _, house: _ } = person;
    }
}
