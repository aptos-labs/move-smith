...
module 0xCAFE::ResourceAcquisition {
    ...
    // Make helper function public and inline to fix visibility errors
    public inline fun nested_acquire(s: &signer, v: u64): u64 {
        acquire(s, v)
    }
...
module 0xCAFE::QuantifiedExpressions {
    ...
    public fun quantified_with_condition(): bool {
        ...
        let q = forall x in 0..20 where x % 2 == 0 { x + 2 > x };
        q
    }
}
...
// Note: Move scripts do not currently support 'while' loops. The following infinite loop causes compile error.
/*
//# run
script {
