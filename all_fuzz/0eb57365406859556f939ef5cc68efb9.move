...
module 0xCAFE::ShadowingAndCapture {
    ...
    public fun foo() {
        let x = 1u8;                           // x should be mutable to get mutable reference
        let inc = &mut x;
...
module 0xCAFE::GenericQueue {
    ...
    public fun dequeue<T>(q: &mut Queue<T>): T {
        ...
        // To avoid copying T (which might not have copy), unpack elem by removing from vector and returning it.
        // So use vector::remove which returns the removed element (and consumes it properly).
        vector::remove(&mut q.elems, 0)
    }

    public fun test_fifo() {
        let q = create<u8>();                // q must be mutable to pass as &mut
        enqueue(&mut q, 10u8);
        enqueue(&mut q, 20u8);
        enqueue(&mut q, 30u8);
...
module 0xCAFE::EnumVariants {
    ...
    public fun test_variants() {
        let e1 = E::V1 { a: 1, b: 2 };
        let e2 = E::V2(10, 20);
        let _e3 = E::V3;         // prefix with _ since unused
        ...
        let (v1_field, v2_first, v2_second) = match (e2) {
            E::V1 { a, _b: _ } => (a, 0u8, 0u8),    // rename unused to _b to suppress warning
