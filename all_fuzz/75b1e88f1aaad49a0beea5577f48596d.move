...
module 0xCAFE::FifoQueue {
    ...
    struct Queue<T: copy + drop + store> has store, drop {
        items: vector<T>,
    ...
    public fun runner(): vector<u8> {
        ...
        // unpack option to consume it properly and avoid drop errors
        while (option::is_some(&item_opt)) {
            // safely unpack the option::Option<u8> to consume item_opt correctly
...
module 0xCAFE::CrossRefStructs {
    ...
    // Add drop ability to support safe drop later
    struct Singleton has key, drop {
    ...
    public fun make_varref_v1<T1: copy + drop, T2: copy + drop>(val1: T1, _val2: T2): VarRef<T1, T2> {
        // consume _val2 so it is not dropped implicitly
        let InnerRef { value: _ } = InnerRef { value: _val2 };
        VarRef::V1 {
    ...
    public fun make_varref_v2<T1: copy + drop, T2: copy + drop>(_val1: T1, val2: T2): VarRef<T1, T2> {
        // consume _val1 so it is not dropped implicitly
        let InnerRef { value: _ } = InnerRef { value: _val1 };
        VarRef::V2 {
            field2: InnerRef<T2> { value: val2 }
        }
    }

    // Runner that tests struct creation by consuming unused vars appropriately
    public fun runner(): bool {
        let s = string::utf8(b"hello");
        let singleton = init_singleton(s);
        // consume singleton properly: unpack the struct to consume
        let Singleton { inner: _ } = singleton;

        let v1 = make_varref_v1<u8, bool>(5u8, true);
        // consume v1 (enum) by matching all variants
        match v1 {
            VarRef::V1 { field1 } => { let InnerRef { value: _ } = field1; },
            VarRef::V2 { field2 } => { let InnerRef { value: _ } = field2; },
        };

        let v2 = make_varref_v2<u8, bool>(6u8, false);
        // consume v2 (enum) similarly
        match v2 {
            VarRef::V1 { field1 } => { let InnerRef { value: _ } = field1; },
            VarRef::V2 { field2 } => { let InnerRef { value: _ } = field2; },
        };
...
module 0xCAFE::UnnecessaryAcquires {
    ...
    // Added key ability to R for move_from/move_to compatibility
