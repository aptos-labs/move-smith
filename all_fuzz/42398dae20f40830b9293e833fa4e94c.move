...
module 0xCAFE::GenericStructModule {
    use std::signer;
    use std::vector; // We can keep this even if unused

    struct GenStruct<T> has store, copy, drop, key {
        value: T
    }

    // Removed 'has copy, drop' from function signature, as Move does not allow it there
    ...
    // Rename function 'exists' to 'exists_gen' to avoid using restricted name
    public fun exists_gen<T>(addr: address): bool {
...
//# run 0xCAFE::GenericStructModule::exists_gen --args 0xDEAD
...
module 0xCAFE::UseGenericModule {
    ...
    public fun store_and_check(s: signer, val: u64): bool {
        ...
        GenericStructModule::exists_gen<u64>(signer::address_of(&s))
    }
